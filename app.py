...
def is_valid_license_format(plate_text):
    """
    Validates if the license plate matches either AB12CD3456 or AB1CDE2345 format
    ...
    """
    # Remove any whitespace from the plate text
    plate_text = ''.join(plate_text.split())
    
    # Define the two acceptable formats using regex
    format1 = r'^[A-Z]{2}\d{2}[A-Z]{2}\d{4}$'  # AB12CD3456
    format2 = r'^[A-Z]{2}\d{1}[A-Z]{3}\d{4}$'  # AB1CDE2345
    
    # Check if the plate matches either format
    return bool(re.match(format1, plate_text) or re.match(format2, plate_text))
    ...
    if video_file is not None:
        ...
        if st.button("Process Video"):
            with st.spinner("Processing video, please wait..."):
                ...
                st.dataframe(results_df)  # Display the results as a table
