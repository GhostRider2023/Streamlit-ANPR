...
def process_video(video_file):
    ...
    while cap.isOpened():
        ...
        # Call model_prediction on each frame
        prediction_results = model_prediction(frame)
        results.append(prediction_results)  # Store results for each frame
...
with body :
    ...
    if st.session_state["state"] == "Uploader":
       ...
       if img is not None:
           if img.name.endswith('.mp4'):
            results = process_video(img)  # Process video and store results in CSV
            st.success("Video processed successfully! Check the CSV for detected number plates.")
           else:
            image = np.array(Image.open(img))
            results = model_prediction(image)  # Process image

    # Assuming `process_video` and `model_prediction` are defined functions
    ...
    #         results = process_video(tfile.name)  # Call the video processing function
    ...
    #             image = np.array(Image.open(img))
    #             results = model_prediction(image)  # Call the image processing function
